...
-- NDT applies an illusion; conflicts with forms like Feral Form / Wolf Form.
local function targetHasFormConflictForNDT(target)
    ...
    if target.ID() == mq.TLO.Me.ID() then
        for _, name in ipairs(forms) do
            if Casting.IHaveBuff(name) then return true end
        end
        return false
    end
    for _, name in ipairs(forms) do
    ...
    if target.ID() == mq.TLO.Target.ID() then
        for _, name in ipairs(forms) do
            if Casting.TargetHasBuff(name, target, false) then return true end
        end
    end
    return false
end
        ...
        ['GroupBuff'] = {
            ...
            },
            {
                ...
                cond = function(self, spell, target)
                    ...
                    -- NDT is an illusion; blocked by Feral Form / wolf forms
                    if targetHasFormConflictForNDT(target) then return false end
