...
-- Keep the stack-guard implementation inside one private scope. RGMercs class
-- configs are very large, and MacroQuest's LuaJIT compiler has tighter limits
-- than desktop syntax checkers. Only the functions used by the class table
-- remain as top-level locals.
local unityHasBlockedChild, runSpeedAlreadyCovered, stackGuardSpellName = (function()

-- Buffs supplied by another class that satisfy one of this config's buff jobs.
-- Add future cross-class alternatives here instead of duplicating target checks
-- inside individual rotation entries.
local BUFF_ALTERNATIVES = {
    RunSpeedBuff = {
        "Flight of Falcons",
        "Spirit of Falcons",
    },
...
-- Newer Unity child effects that satisfy the same job as older children.
-- Keys are the older child spell names reported by Talisman of the Heroic.
local UNITY_CHILD_ALTERNATIVES = {
    ["heroic focusing"] = {
        "Spirit's Focusing XIV",
    },
    ["spirit of lost legends"] = {
        "Spirit of Fortitude XVIII",
    },
    ["vampyre focusing"] = {
        "Spirit's Focusing XIV",
    },
...
-- Known Unity parents are listed explicitly because MQ's Trigger(index)
-- results are not reliable enough to use as the only source for every spell.
-- The dynamic trigger scan below remains as a fallback for older Unity spells.
local UNITY_PARENT_CHILDREN = {
    ["talisman of unity x"] = {
        "Mammoth's Unity",
        "Preeminent Unity",
        "Spirit's Focusing XIV",
...
local function targetHasNamedBuff(checkTarget, buffName)
    if not (checkTarget and checkTarget()) then return false end

    local buffSpell = mq.TLO.Spell(buffName)
    local buffId = buffSpell and buffSpell() and buffSpell.ID() or 0

    -- Use RGMercs' normal remote-buff path first. AddedBuffCheck consults the
    -- local windows, RGMercs Actor heartbeats, DanNet, and visible targets.
    -- It returns false when the requested buff is already present.
    if buffId > 0 and type(Casting.AddedBuffCheck) == "function" then
        local ok, buffNeeded = pcall(function()
            return Casting.AddedBuffCheck(buffId, checkTarget)
        end)
        if ok and buffNeeded == false then return true end
    end

    local function spellMatches(id)
        id = tonumber(id)
        if not id then return false end
        if buffId > 0 and id == buffId then return true end

        local spell = mq.TLO.Spell(id)
        local spellName = spell and spell() and spell.Name() or nil
        return spellName == buffName
    end

    -- This client can read both of its own effect windows directly.
    if checkTarget.ID() == mq.TLO.Me.ID() then
        return Casting.IHaveBuff(buffName) or (buffId > 0 and Casting.IHaveBuff(buffId))
    end

    -- Prefer the RGMercs Actor heartbeat. It works without changing targets and
    -- includes both long buffs and short-duration songs.
    local checkName = checkTarget.CleanName() or checkTarget.DisplayName() or ""
    local isPet = Targeting.TargetIsType("Pet", checkTarget)
    local heartbeatName = (isPet and checkTarget.Master()) and checkTarget.Master.DisplayName() or checkName
    local heartbeat = Comms.GetPeerHeartbeatByName(heartbeatName)
    if heartbeat and heartbeat.Data then
        local listsToCheck = { heartbeat.Data.Buffs, heartbeat.Data.Songs }
        if isPet then
            table.insert(listsToCheck, heartbeat.Data.PetBuffs)
        end

        for _, buffList in ipairs(listsToCheck) do
            if buffList then
                for _, activeSpellId in ipairs(buffList) do
                    if spellMatches(activeSpellId) then
                        return true
                    end
                end
            end
    ...
    -- CastTracker Actors and RGMercs Actors use different mailboxes. If the
    -- RGMercs heartbeat is unavailable, ask the recipient through DanNet.
    if buffId > 0 and checkName ~= "" then
        local peerAvailable = false
        pcall(function()
            peerAvailable = mq.TLO.DanNet(checkName)() ~= nil
        end)
        if peerAvailable then
            local ok, result = pcall(function()
                return DanNet.query(
                    checkName,
                    string.format("Me.FindBuff[id %d]", buffId),
                    1000
                )
            end)
            result = ok and tostring(result or "") or ""
            if result:lower() == buffName:lower() then
                return true
            end
        end
    end

    -- Fallback for a visible current target or an out-of-combat target that the
    -- Shaman can safely select to populate its buff data.
    if checkTarget.ID() ~= mq.TLO.Target.ID() then
        if (mq.TLO.Me.CombatState() or ""):lower() == "combat" then
            return false
        end
        Targeting.SetTarget(checkTarget.ID(), false)
    end

    local function targetEffectExists(effect)
        local ok, found = pcall(function()
            return mq.TLO.Target.Buff(effect)() ~= nil
        end)
        return ok and found or false
    end

    return targetEffectExists(buffName) or (buffId > 0 and targetEffectExists(buffId))
end

local function targetHasBuffAlternative(checkTarget, buffJob)
    for _, buffName in ipairs(BUFF_ALTERNATIVES[buffJob] or {}) do
        if targetHasNamedBuff(checkTarget, buffName) then
...
local function spellName(spell)
    if not (spell and spell()) then return nil end
    return spell.Name() or spell.RankName()
end

local function normalizedSpellKey(name)
    name = tostring(name or ""):lower()
    name = name:gsub("%s+rk%.%s+[ivx]+$", "")
...
-- Returns true when the child can land, false when an existing buff blocks it,
-- and nil when the recipient's stacking state cannot be determined.
local function targetSpellCanLand(checkTarget, childSpell)
    if not (checkTarget and checkTarget()) then return nil end
    if not (childSpell and childSpell()) then return nil end

    local childId = tonumber(childSpell.ID()) or 0
    if childId <= 0 then return nil end

    if checkTarget.ID() == mq.TLO.Me.ID() then
        local ok, stacks = pcall(function() return childSpell.Stacks() end)
        if not ok then return nil end
        return parseBoolean(stacks)
    end

    local checkName = checkTarget.CleanName() or checkTarget.DisplayName() or ""
    local heartbeat = Comms.GetPeerHeartbeatByName(checkName)
    if heartbeat and heartbeat.Data then
        local foundList = false
        for _, activeList in ipairs({ heartbeat.Data.Buffs, heartbeat.Data.Songs }) do
            if activeList then
                foundList = true
                for _, activeSpellId in ipairs(activeList) do
                    activeSpellId = tonumber(activeSpellId)
                    if activeSpellId then
                        local ok, stacks = pcall(function()
                            return childSpell.StacksWith(activeSpellId)()
                        end)
                        if ok and parseBoolean(stacks) == false then
                            return false, mq.TLO.Spell(activeSpellId).Name()
                        end
                    end
                end
            end
        end
        if foundList then return true end
    end

    if checkName ~= "" then
        local okAvailable, peerAvailable = pcall(function()
            return mq.TLO.DanNet(checkName)()
        end)
        if okAvailable and peerAvailable then
            local okQuery, result = pcall(function()
                return DanNet.query(
                    checkName,
                    string.format("Spell[%d].Stacks", childId),
                    1000
                )
            end)
            if okQuery then
                local stacks = parseBoolean(result)
    ...
    if checkTarget.ID() == mq.TLO.Target.ID() then
        local ok, stacks = pcall(function() return childSpell.StacksTarget() end)
        if ok then return parseBoolean(stacks) end
    end

    return nil
end

local function unityChildNameCovered(target, childName)
    if targetHasNamedBuff(target, childName) then
        return true
    end

    for _, alternativeName in ipairs(UNITY_CHILD_ALTERNATIVES[normalizedSpellKey(childName)] or {}) do
        if targetHasNamedBuff(target, alternativeName) then
            Logger.log_verbose(
                "GroupFocusSpell: %s on %s is satisfied by stronger %s.",
                childName,
                target.CleanName() or "target",
                alternativeName
            )
            return true
        end
    ...
    local childSpell = mq.TLO.Spell(childName)
    if childSpell and childSpell() and type(Casting.ResolveBuffCheck) == "function" then
        local childId = tonumber(childSpell.ID()) or 0
        if childId > 0 then
            local ok, shouldCast = pcall(function()
                -- skipTriggerCheck=true because this is already a single Unity
                -- child. false means present, blocked, or unable to stack.
                return Casting.ResolveBuffCheck(childId, target, false, true)
            end)
            if ok then
                if shouldCast == false then
                    Logger.log_verbose(
                        "GroupFocusSpell: RGMercs reports %s is present or blocked on %s; counting it as covered.",
                        childName,
                        target.CleanName() or "target"
                    )
                    return true
    ...
    local canLand, blockerName = targetSpellCanLand(target, childSpell)
    if canLand == false then
        Logger.log_verbose(
            "GroupFocusSpell: %s cannot stack on %s%s; counting it as covered.",
            childName,
            target.CleanName() or "target",
            blockerName and string.format(" because of %s", blockerName) or ""
        )
        return true
    end
...
local function unityChildCovered(target, triggerSpell)
    local triggerName = spellName(triggerSpell)
    if not triggerName then return false end
    local baseName = triggerSpell.BaseName() or triggerName
    return unityChildNameCovered(target, baseName)
        or (normalizedSpellKey(baseName) ~= normalizedSpellKey(triggerName)
            and unityChildNameCovered(target, triggerName))
end

-- Unity is a single cast that attempts several child buffs. In strict mode,
-- one duplicate or non-stacking child blocks the entire parent cast. This
-- prevents a useful child from making RGMercs cast Unity while the remaining
-- children generate duplicate/stronger-buff failures.
local function unityHasBlockedChild(target, unitySpell)
    if not (unitySpell and unitySpell()) then return false end

    local parentName = spellName(unitySpell)
    local knownChildren = UNITY_PARENT_CHILDREN[normalizedSpellKey(parentName)]
    if knownChildren then
        for _, childName in ipairs(knownChildren) do
            if unityChildNameCovered(target, childName) then
    ...
    local numEffects = unitySpell.NumEffects() or 0
    for effectSlot = 1, numEffects do
        local triggerSpell = unitySpell.Trigger(effectSlot)

        -- Unity trigger slots are not guaranteed to be contiguous. Ignore an
        -- empty/Unknown slot and continue checking the remaining effect slots.
        if triggerSpell and triggerSpell() and (triggerSpell.ID() or 0) > 0 then
            if unityChildCovered(target, triggerSpell) then
                return true, spellName(triggerSpell)
            end
        end
    end
...
local function targetHasFalconByScan(checkTarget)
    if not (checkTarget and checkTarget()) then return false, nil end

    -- Self: scan buff window for any Falcon-line name
    if checkTarget.ID() == mq.TLO.Me.ID() then
        local maxSlots = mq.TLO.Me.MaxBuffSlots() or 42
        for i = 1, maxSlots do
            local n = mq.TLO.Me.Buff(i).Name()
            if n and n:find("Falcons", 1, true) then
                return true, n
            end
        end
        return false, nil
    ...
    -- Actor heartbeat: resolve each active buff ID to a name and look for Falcon
    local checkName = checkTarget.CleanName() or checkTarget.DisplayName() or ""
    local isPet = Targeting.TargetIsType("Pet", checkTarget)
    local heartbeatName = (isPet and checkTarget.Master()) and checkTarget.Master.DisplayName() or checkName
    local heartbeat = Comms.GetPeerHeartbeatByName(heartbeatName)
    if heartbeat and heartbeat.Data then
        local lists = { heartbeat.Data.Buffs, heartbeat.Data.Songs }
        if isPet then table.insert(lists, heartbeat.Data.PetBuffs) end
        for _, buffList in ipairs(lists) do
            if buffList then
                for _, activeSpellId in ipairs(buffList) do
                    activeSpellId = tonumber(activeSpellId)
                    if activeSpellId then
                        local spell = mq.TLO.Spell(activeSpellId)
                        local spellName = spell and spell() and (spell.Name() or spell.RankName()) or nil
                        if spellName and spellName:find("Falcons", 1, true) then
    ...
    -- DanNet name lookup (works across ranks; avoid id-only FindBuff)
    if checkName ~= "" then
        local peerAvailable = false
        pcall(function()
            peerAvailable = mq.TLO.DanNet(checkName)() ~= nil
        ...
        if peerAvailable then
            for _, falconName in ipairs({ "Flight of Falcons", "Spirit of Falcons" }) do
                local ok, result = pcall(function()
                    return DanNet.query(checkName, string.format("Me.Buff[%s]", falconName), 1000)
                end)
                result = ok and tostring(result or "") or ""
                if result ~= "" and result:lower() ~= "null" and result:lower() ~= "nil" then
                    return true, falconName
                end
            end
        end
    end

    -- Current target buff window (Target.Buffs is not valid on all MQ builds)
    if checkTarget.ID() == mq.TLO.Target.ID() then
        local maxSlots = mq.TLO.Me.MaxBuffSlots() or 42
        for i = 1, maxSlots do
            local ok, n = pcall(function() return mq.TLO.Target.Buff(i).Name() end)
            if ok and n and n:find("Falcons", 1, true) then
                return true, n
            end
        end
    end
...
local function runSpeedAlreadyCovered(target, proposedBuffName)
    if proposedBuffName and targetHasNamedBuff(target, proposedBuffName) then
        Logger.log_verbose(
            "RunSpeedBuff: %s is already present on %s, skipping duplicate.",
            proposedBuffName,
            target.CleanName() or "target"
        )
        return true
    ...
    local covered, buffName = targetHasBuffAlternative(target, "RunSpeedBuff")
    if covered then
        Logger.log_verbose(
            "RunSpeedBuff: %s is already present on %s, skipping Shaman run speed.",
            buffName,
            target.CleanName() or "target"
        )
        return true
    ...
    -- Explicit Falcon scan: exact-ID/name checks miss ranks and Spirit vs Flight
    local hasFalcon, falconName = targetHasFalconByScan(target)
    if hasFalcon then
        Logger.log_verbose(
            "RunSpeedBuff: %s detected on %s, skipping Shaman run speed (Tala'Tak).",
            falconName or "Falcon",
            target.CleanName() or "target"
        )
        return true
    end
...
return unityHasBlockedChild, runSpeedAlreadyCovered, spellName
end)()

print(string.format(
    "[CastTracker] RGMercs SHM Stack Guard 10 (quiet strict Unity) loaded on %s.",
    mq.TLO.Me.CleanName() or "this Shaman"
))

-- Group Shrink is group/self only. Never cast for zone peers, raid-only, or unknown Height().
local function isOurGroupMember(target)
    if not (target and target()) then return false end
    local tid = target.ID()
    if tid == mq.TLO.Me.ID() then return true end

    local count = mq.TLO.Group.Members() or 0
    for i = 1, count do
        local m = mq.TLO.Group.Member(i)
        if m() and m.ID() == tid then return true end
    end

    -- Name fallback (some peers resolve better by name than ID briefly)
    local name = target.CleanName() or target.DisplayName()
    if name and name ~= "" then
        local gm = mq.TLO.Group.Member(name)
        if gm and gm() and gm.ID() == tid then return true end
    end

    return false
end

local function targetInShrinkRange(target, maxRange)
    if not (target and target()) then return false end
    if target.ID() == mq.TLO.Me.ID() then return true end
    maxRange = tonumber(maxRange) or 100
    local dist = 9999
    pcall(function() dist = tonumber(target.Distance3D()) or 9999 end)
    return dist <= maxRange
end

local function targetNeedsShrink(target, maxRange)
    -- Hard gate: out of group = never
    if not isOurGroupMember(target) then return false end
    if not targetInShrinkRange(target, maxRange or 100) then return false end
    local h = 0
    pcall(function() h = tonumber(target.Height()) or 0 end)
    if h <= 0 then return false end
    return h > 2.2
end

-- Group AA shrinks the whole group in one cast.
-- Only count tall members that are in range (default 100).
local function groupNeedsShrink(maxRange)
    maxRange = tonumber(maxRange) or 100
    if (tonumber(mq.TLO.Me.Height()) or 0) > 2.2 then return true end
    local count = mq.TLO.Group.Members() or 0
    for i = 1, count do
        local m = mq.TLO.Group.Member(i)
        if m() and not m.OtherZone() then
            local dist = 9999
            pcall(function() dist = tonumber(m.Distance3D()) or 9999 end)
            if dist <= maxRange then
                local h = 0
                pcall(function() h = tonumber(m.Height()) or 0 end)
                if h > 2.2 then return true end
            end
        end
    end
    return false
...
local _ClassConfig = {
    _version              = "2.2.6 - Live (CastTracker Stack Guard 10 - Quiet Strict Unity)",
    ...
    ['Cures']             = {
        ...
        -- -- since it is only run at startup, i'm fine with it. - Cannonballdex 8/29/25
    ...
    ['RotationOrder']     = {
        ...
        { --Mythic Glyph line: which glyph fires (and its trigger) is chosen via GlyphType; runs regardless of Heal mode
            name = 'EmergencyGlyph',
            state = 1,
            steps = 1,
            doFullRotation = true,
    ...
    ['Rotations']         = {
        ...
        ['GroupBuff'] = {
            ...
            { --Used on the entire group
                ...
                type = "Spell",
                cond = function(self, spell, target)
                    local blocked, blockedChild = unityHasBlockedChild(target, spell)
                    if blocked then
                        Logger.log_verbose(
                            "Stack Guard 10 skipped %s on %s because %s is already present or blocked.",
                            stackGuardSpellName(spell) or "Unity",
                            target.CleanName() or "target",
                            blockedChild or "a child buff"
                        )
                        return false
                    end
                    local shouldCast = Casting.GroupBuffCheck(spell, target)
                    if shouldCast then
                        Logger.log_verbose(
                            "Stack Guard 10 allowing %s on %s because no child buff is present or blocked.",
                            stackGuardSpellName(spell) or "Unity",
                            target.CleanName() or "target"
                        )
                    end
            ...
            },
            {
                ...
                cond = function(self, aaName, target)
                    ...
                    local trigger = aaSpell and aaSpell() and aaSpell.Trigger(1) or nil
                    local triggerName = trigger and trigger() and trigger.RankName() or nil
                    if runSpeedAlreadyCovered(target, triggerName) then return false end
                    return Casting.GroupBuffAACheck(aaName, target)
                end,
            },
            {
                ...
                cond = function(self, spell, target)
                    local spellName = spell and spell() and spell.RankName() or nil
                    if runSpeedAlreadyCovered(target, spellName) then return false end
                    return Casting.GroupBuffCheck(spell, target)
                end,
            },
            {
                ...
                cond = function(self, aaName, target)
                    -- Out of group targets from ActorBuffScope / raid list: never
                    if not isOurGroupMember(target) then return false end
                    -- Only trigger the group AA once from self target to avoid repeat casts
                    -- while iterating every buffable ID
                    ...
                    pcall(function()
                        ...
                        if aa and aa() then
                            local sp = aa.Spell
                            if sp and sp() then
                                maxRange = sp.AERange() or sp.MyRange() or 100
                            end
                        end
                    end)
                    if maxRange <= 0 then maxRange = 100 end
                    return groupNeedsShrink(maxRange)
                end,
            },
            {
                name = "ShrinkSpell",
                ...
                cond = function(self, spell, target)
                    ...
                    if spell and spell() then
                        maxRange = spell.MyRange() or spell.AERange() or 100
                        if maxRange <= 0 then maxRange = 100 end
                    end
                    -- Spell path: group/self only, in range, and actually tall
                    return targetNeedsShrink(target, maxRange)
                end,
            },
            {
                name = "LowLvlHPBuff",
