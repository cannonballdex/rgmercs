...
-- Flight/Spirit of Falcons are the same run-speed job; skip if either is present.
local function targetHasAnyFalconBuff(target)
    if not (target and target()) then return false end
    if target.ID() == mq.TLO.Me.ID() then
        return Casting.IHaveBuff("Flight of Falcons")
            or Casting.IHaveBuff("Spirit of Falcons")
    end
    ...
    if target.ID() == mq.TLO.Target.ID() then
        return Casting.TargetHasBuff("Flight of Falcons", target, false)
            or Casting.TargetHasBuff("Spirit of Falcons", target, false)
    end
    -- ID-based presence via AddedBuffCheck (false means present or blocked)
    for _, name in ipairs({ "Flight of Falcons", "Spirit of Falcons" }) do
        local sp = mq.TLO.Spell(name)
        local id = sp and sp() and sp.ID() or 0
        if id > 0 then
            local ok2, needed = pcall(function() return Casting.AddedBuffCheck(id, target) end)
            if ok2 and needed == false then
                -- Could be present OR blocked by the other Falcon — either way don't cast
                return true
            end
        end
    end
...
local _ClassConfig = {
    _version              = "2.1.1 - Live",
    _author               = "Cannonballdex, Grimmier",
    ['ModeChecks']        = {
