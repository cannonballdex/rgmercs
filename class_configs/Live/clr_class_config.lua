        ...
        ['BigHeal(77+)'] = {
            ...
            },
            {
                ...
                cond = function(self, aaName, target)
                    ...
                    local aggroHolderId = mq.TLO.Target.AggroHolder.ID() or 0
                    local mainAssistId = Core.GetMainAssistId() or 0

                    return targetId > 0 and targetId == aggroHolderId and targetId ~= mainAssistId
                end,
            },
            {
    ...
    ['Rotations']         = {
        ...
        ['GroupBuff'] = {
            ...
            },
            {
                ...
                type = "Spell",
                cond = function(self, spell, target)
                    local aegoSymbolSetting = Config:GetSetting('AegoSymbol')
                    local isSingleTarget = (spell.TargetType() or ""):lower() == "single"

                    if aegoSymbolSetting == 1 or aegoSymbolSetting == 4 then return false end
                    if isSingleTarget and target.ID() ~= Core.GetMainAssistId() then return false end
