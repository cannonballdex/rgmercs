...
local RUN_SPEED_ALTERNATIVES = {
    "Flight of Falcons",
    "Spirit of Falcons",
}

local function validSpellId(spell)
    if not (spell and spell()) then return 0 end
    return tonumber(spell.ID()) or 0
end

local function targetName(target)
    if not (target and target()) then return "target" end
    return target.CleanName() or target.DisplayName() or "target"
end

local function buffIsPresent(target, spell)
    local spellId = validSpellId(spell)
    if spellId <= 0 then return false end
    return not Casting.AddedBuffCheck(spellId, target)
end

local function targetHasFalconBuff(target)
    ...
    if target.ID() == mq.TLO.Target.ID() then
        local maxSlots = mq.TLO.Target.Buffs() or 42
        for i = 1, maxSlots do
            local n = mq.TLO.Target.Buff(i).Name()
            if n and n:find("Falcons", 1, true) then
                return true, n
            end
        end
    ...
    local ok, found = pcall(function()
        local b = target.Buff("Flight of Falcons")
        if b and b() then return true end
        b = target.Buff("Spirit of Falcons")
        return b and b() ~= nil
    end)
    return ok and found
end

local function runSpeedAlreadyCovered(spell, target)
    if buffIsPresent(target, spell) then
        Logger.log_verbose(
            "RunSpeedBuff: %s is already present on %s, skipping duplicate.",
            spell.RankName() or spell.Name() or "run speed",
            targetName(target)
        )
        return true
    end

    for _, alternativeName in ipairs(RUN_SPEED_ALTERNATIVES) do
        local alternativeSpell = mq.TLO.Spell(alternativeName)
        if buffIsPresent(target, alternativeSpell) then
            Logger.log_verbose(
                "RunSpeedBuff: %s is already present on %s, skipping Beastlord run speed.",
                alternativeName,
                targetName(target)
            )
            return true
    ...
    -- Falcon overwrites Tala'Tak; do not cast Tala'Tak while Falcon is present
    local has, name = targetHasFalconBuff(target)
    if has then
        Logger.log_verbose(
            "RunSpeedBuff: %s on %s, skipping Beastlord run speed.",
            name or "Falcon",
            targetName(target)
        )
        return true
...
return {
    ...
    ['ItemSets']          = {
        ['Epic'] = {
            "Savage Lord's Totem",             -- Epic 1.5
            "Spiritcaller Totem of the Feral", -- Epic 2.0
        },
        -- Omens / Gates-era chests used by the OoW_Chest burn click
        ['OoW_Chest'] = {
    ...
    ['AbilitySets']       = {
        -- AERoar (Timer 11 PBAE) is fully wired: AbilitySet + DoAERoar setting + rotation/spell-list entries.
        -- When DoAERoar is enabled it replaces Icelance2 in the loadout.
        ['SwarmPet'] = {
    ...
    ['Rotations']         = {
        ...
        ['DPS'] = {
            ...
            },
            {
                ...
                cond = function(self, spell, target)
                    ...
                    -- Note: there are edge cases where Growl isn't needed; skipping them is low benefit vs complexity.
        ...
        ['GroupBuff'] = {
            {
                ...
                cond = function(self, spell, target)
                    if runSpeedAlreadyCovered(spell, target) then return false end
        ...
        ['PetBuff'] = {
            ...
            },
            {
                ...
                load_cond = function(self) return Config:GetSetting('DoRunSpeed') end,
                cond = function(self, spell)
                    if runSpeedAlreadyCovered(spell, mq.TLO.Me.Pet) then return false end
                    return Casting.PetBuffCheck(spell)
                end,
            },
            {
    ...
    ['DefaultConfig']     = {
        -- PetProcChoice is already a combo (Slow Proc vs Snare/Hobble).
        -- Damage + Heal procs remain always-on; they stack and are generally desirable.
        ['Mode']           = {
            DisplayName = "Mode",
            Category = "Combat",
        ...
        ['PetProcChoice']  = {
            ...
            Category = "Pet Buffs",
            Index = 102,
            Tooltip = "Choose the extra proc buff for your pet:\n" ..
                "1) Slow Proc - Steeltrap/Lockfang Jaws line (slow on hit).\n" ..
                "2) Snare - Hobble of Spirits AA (snare on hit).\n" ..
                "Damage and Heal procs are always applied separately.",
            Type = "Combo",
            ComboOptions = { 'Slow Proc', 'Snare (Hobble)', },
            Default = 1,
            Min = 1,
            Max = 2,
            RequiresLoadoutChange = true,
