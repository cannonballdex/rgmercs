...
local _ClassConfig = {
    ...
    ['Rotations']       = {
        ...
        ['DPS(MobHighHP)']  = {
            ...
            },
            {
                ...
                cond = function(self, spell, target)
                    ...
                    return not Casting.IHaveBuff(spell.Name() .. " Recourse")
                        and (mq.TLO.Target.PctMana() or 0) > 0
                        and Casting.GroupLowManaCount(40) > 2
                end,
            },
            {
                name = "VenomDot",
        ...
        ['Burn']            = {
            ...
            -- 1) Debuffs that amplify the rest of the burn
            -- 2) Major personal damage amplifiers / twincast
            -- 3) Long-duration high-impact AAs
            -- 4) Pet / swarm boosts
            -- 5) Utility / situational
            -- 6) Clickies & minor buffs
            {
                name = "Scent of Thule",
                type = "AA",
                cond = function(self, aaName, target)
                    return Globals.AutoTargetIsNamed and Casting.DetAACheck(aaName)
                end,
            },
            {
            ...
            { -- Chest Click – name_func prevents errors when slot is empty
        ...
        ['PetSummon']       = {
            -- Pet spell lists (WarPetSpell / RogPetSpell) are ordered highest rank first.
            -- Casting resolves the first known spell, so leveling characters automatically
            -- receive the best pet they can currently cast without changing options.
            {
                name_func = function(self)
                    local petType = self.ClassConfig.DefaultConfig.PetType.ComboOptions[Config:GetSetting('PetType')] or "War"
                    return string.format("%sPetSpell", petType)
                end,
                type = "Spell",
