...
-- Mod rods / modulation shards are LORE. Never summon if the target already has one.
-- Spell Base(1) item IDs are unreliable across ranks, so also check common names.
local MOD_ROD_ITEM_NAMES = {
    -- Live summoned rods (common ranks)
    "Summoned: Modulation Rod IX",
    "Summoned: Modulation Rod VIII",
    "Summoned: Modulation Rod VII",
    "Summoned: Modulation Rod VI",
    "Summoned: Modulation Rod V",
    "Summoned: Modulation Rod IV",
    "Summoned: Modulation Rod III",
    "Summoned: Modulation Rod II",
    "Summoned: Modulation Rod",
...
-- Broad name match for any "Summoned: Modulation Rod ..." rank the list might miss
local function nameLooksLikeModRod(itemName)
    if not itemName then return false end
    local n = itemName:lower()
    if n:find("modulation rod", 1, true) then return true end
    if n:find("modulating rod", 1, true) then return true end
    if n:find("modulation shard", 1, true) then return true end
    if n:find("transvergence", 1, true) and n:find("rod", 1, true) then return true end
    return false
end

local function inventoryCountById(itemId)
    itemId = tonumber(itemId) or 0
    if itemId <= 0 then return 0 end
    return tonumber(mq.TLO.FindItemCount(itemId)()) or 0
end

local function inventoryCountByName(itemName)
    if not itemName or itemName == "" then return 0 end
    return tonumber(mq.TLO.FindItemCount(string.format("=%s", itemName))()) or 0
end

local function selfHasModRod(itemId)
    if inventoryCountById(itemId) > 0 then return true end
    for _, name in ipairs(MOD_ROD_ITEM_NAMES) do
        if inventoryCountByName(name) > 0 then return true end
    end
    -- Scan main inventory slots for any Modulation Rod rank
    for slot = 1, 32 do
        local ok, n = pcall(function() return mq.TLO.Me.Inventory(slot).Name() end)
        if ok and nameLooksLikeModRod(n) then return true end
        local bagSize = 0
        pcall(function() bagSize = mq.TLO.Me.Inventory(slot).Container() or 0 end)
        if bagSize and bagSize > 0 then
            for b = 1, bagSize do
                local ok2, bn = pcall(function() return mq.TLO.Me.Inventory(slot).Item(b).Name() end)
                if ok2 and nameLooksLikeModRod(bn) then return true end
            end
        end
    end
    return false
...
local function peerItemCount(peerName, query)
    local ok, result = pcall(function()
        return DanNet.query(peerName, query, 1000)
    end)
    if not ok or result == nil then return nil end
    local s = tostring(result):lower()
    if s == "" or s == "null" or s == "nil" then return nil end
    return tonumber(result)
end

local function peerHasModRod(peerName, itemId)
    if not peerName or peerName == "" then return true end -- fail closed: assume has rod

    itemId = tonumber(itemId) or 0
    if itemId > 0 then
        local c = peerItemCount(peerName, string.format("FindItemCount[%d]", itemId))
        if c and c > 0 then return true end
    end

    for _, name in ipairs(MOD_ROD_ITEM_NAMES) do
        local c = peerItemCount(peerName, string.format("FindItemCount[=%s]", name))
        if c and c > 0 then return true end
    end

    -- Partial name queries catch ranks not in the static list
    for _, partial in ipairs({ "Summoned: Modulation Rod", "Modulation Rod", "Modulating Rod", "Modulation Shard" }) do
        local c = peerItemCount(peerName, string.format("FindItemCount[%s]", partial))
        if c and c > 0 then return true end
    end

    -- If DanNet is down / queries fail, do not summon (avoids LORE duplicate spam)
    if itemId > 0 then
        local c = peerItemCount(peerName, string.format("FindItemCount[%d]", itemId))
...
--- Returns true only when it is safe to summon a mod rod for this target.
local function targetNeedsModRod(target, itemId)
    if not (target and target()) then return false end
    if target.ID() == mq.TLO.Me.ID() then
        return not selfHasModRod(itemId)
    end
    local peerName = target.CleanName() or target.DisplayName()
    return not peerHasModRod(peerName, itemId)
end
...
_ClassConfig    = {
    ...
    ['AbilitySets']       = { --TODO: Look into new TOB item summons (Boiling Orb?)
        ...
        ['ManaRodSummon'] = { -- Level 44 - 105
            --- Single-target only. Mass Transvergence gives a rod to the whole group and
            --- spams "Duplicate items not allowed" on anyone who already has a LORE rod.
            "Modulating Rod",
        },
        ['SelfManaRodSummon'] = {
        ...
        ['Summon ModRods'] = {
            {
                ...
                cond = function(self, aaName, target)
                    ...
                    -- Only cast if this target does not already have a rod/shard
                    return targetNeedsModRod(target, modRodItem)
                end,
            ...
            },
            {
                ...
                cond = function(self, spell, target)
                    ...
                    if spellName:find("Mass", 1, true) then return false end
                    if (mq.TLO.Cursor.ID() or 0) ~= 0 then return false end
                    local myId = Casting.GetUseableSpellId(spell)
                    local modRodItemId = mq.TLO.Spell(myId).Base(1)() or 0
                    -- Only cast if this target does not already have a rod
                    return targetNeedsModRod(target, modRodItemId)
                end,
                post_activate = function(self, spell, success)
                    if success then
                        Core.SafeCallFunc("Autoinventory", ClassHelpers(self).HandleItemSummon, self, spell, "personal")
            ...
            },
            {
                ...
                cond = function(self, spell, target, combat_state)
                    ...
                    local myId = Casting.GetUseableSpellId(spell) -- Adjust for possible unsubbed accounts
                    local modRodItemId = mq.TLO.Spell(myId).Base(1)() or 0
                    if modRodItemId <= 0 or (mq.TLO.Cursor.ID() or 0) ~= 0 then return false end
                    if not targetNeedsModRod(target, modRodItemId) then return false end
                    return not (combat_state == "Combat" and mq.TLO.Me.PctMana() > Config:GetSetting('GroupManaPct'))
                end,
                post_activate = function(self, spell, success)
                    if success then
                        Core.SafeCallFunc("Autoinventory", ClassHelpers(self).HandleItemSummon, self, spell, "personal")
