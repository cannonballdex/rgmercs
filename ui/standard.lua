...
local StandardUI    = { _version = '1.0', _name = "StandardUI", _author = 'Cannonballdex', }
...
function StandardUI:RenderTarget()
    ...
    if not Config.DbConsistencyCheckPass then
        warningMessage = warningMessage and warningMessage .. "\n" or "" .. "Database Consistency Check Failed! Please check logs and report to Cannonballdex!"
    end

    if not Config.UnitTestsPass then
        warningMessage = warningMessage and warningMessage .. "\n" or "" .. "Unit Tests Failed! Please check logs and report to Cannonballdex!"
    end
