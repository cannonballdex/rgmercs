        ...
        if ImGui.BeginChild("##RGMercsMainBody", ImVec2(0, 0), bit32.bor(ImGuiChildFlags.None), bit32.bor(ImGuiWindowFlags.None)) then
            ...
            if ImGui.BeginTable("##MainInfoTable", 2, bit32.bor(ImGuiTableFlags.BordersInner, ImGuiTableFlags.SizingFixedFit)) then
                ...
                Ui.RenderColoredText(mq.TLO.Stick.Active() and Globals.Constants.Colors.ConditionPassColor or Globals.Constants.Colors.ConditionDisabledColor,
                    "%s", (mq.TLO.Stick.Active() and (Movement.GetLastStickCmd and Movement:GetLastStickCmd() or "N/A") or "N/A"))
                ImGui.SameLine()
                Ui.RenderText("] ")
                ImGui.SameLine()
                Ui.RenderText("<")
                ImGui.SameLine()
                Ui.RenderColoredText(Globals.Constants.Colors.LightBlue, "%s", (Movement.GetTimeSinceLastStick and Movement:GetTimeSinceLastStick()) or "0s")
                ImGui.SameLine()
                Ui.RenderText(">")
                ImGui.TableNextColumn()
                Ui.RenderText("Last Nav")
                local lastNav, lastNavTracer = nil, nil
                if Movement.GetLastNavCmd then
                    lastNav, lastNavTracer = Movement:GetLastNavCmd()
                end
                ImGui.TableNextColumn()
                ...
                Ui.RenderText("<")
                ImGui.SameLine()
                Ui.RenderColoredText(Globals.Constants.Colors.LightBlue, "%s", (Movement.GetTimeSinceLastNav and Movement:GetTimeSinceLastNav()) or "0s")
                ImGui.SameLine()
                Ui.RenderText(">")
                ImGui.TableNextColumn()
    ...
    if not Globals.Minimized then
        ...
        if shouldDrawGUI then
            if ImGui.BeginChild("##RGMercsMainHeader", ImVec2(0, 0), bit32.bor(ImGuiChildFlags.AlwaysAutoResize, ImGuiChildFlags.AutoResizeY),
                ...
                local imgDisplayed = Globals.LastBurnCheck and ImageUI.burnImg or ImageUI.cannonballdexImg
                if imgDisplayed then
