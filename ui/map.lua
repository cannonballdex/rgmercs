...
local MapUI           = { _version = '1.0', _name = "MapUI", _author = 'Cannonballdex', }
MapUI.__index         = MapUI
...
local function getEditorBounds()
    ...
    local myX = -(me.X() or 0)
    local myY = me.Y() or 0
    local halfRange = math.max(Config:GetSetting('TargetRadius', true) or 300, 120)
...
function MapUI:RenderCanvas(canvasWidth, canvasHeight)
    ...
    local starColor = ImGui.GetColorU32(ImVec4(1.0, 0.84, 0.0, 1.0))
    local namedColor = ImGui.GetColorU32(ImVec4(0.30, 1.00, 0.45, 1.0))
    ...
    for i = 1, npcsMaxRenderCount do
        ...
        if spawn.Type() == "NPC" and not spawn.Dead() then
            ...
            if isChest or isNamed then
                local markColor = isChest and starColor or namedColor
                local starPts = {}
                for k = 0, 9 do
                ...
                drawList:AddConvexPolyFilled(starPts, markColor)
            else
                drawList:AddCircleFilled(ImVec2(sx, sy), 3, ImGui.GetColorU32(ImVec4(r, g, b, 0.75)), 10)
            end
            drawList:AddText(ImVec2(sx + 5, sy - 7), ImGui.GetColorU32(ImVec4(r, g, b, 0.85)),
    ...
    if isHovered then
        if hoveredSpawn then
            ...
            ImGui.SetTooltip('World: %.1f, %.1f | Drag to pan | Right-click a mob to target', worldX, worldY)
        end
    end
