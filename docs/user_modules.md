...
| `Shutdown()` | On unload. Release your own state and tear down anything you registered yourself (see below). |

## Where things go

- **Settings** go in `Module.DefaultConfig`, keyed by setting name. Define the table even if it is empty — a module without one will not load. Each entry carries `DisplayName`, `Tooltip`, `Default` and friends, and every entry that is not `Type = "Custom"` must also carry a `Category`, which is what the settings UI groups it under. A category not already wired into a settings tab elsewhere in RGMercs falls back to the "Uncategorized" catch-all — that is fine, it will still show up.
...
- **Events, binds, actors and ImGui windows** are plain `mq.event`, `mq.bind`, `mq.imgui.init` and actor registrations — register them in `Init()`. This RGMercs build does not auto-track or tear these down for you on unload/disable, so unregister anything you registered (`mq.unevent`, `mq.unbind`, `mq.imgui.destroy`, `dropbox:unregister()`) yourself in `Shutdown()`. Skipping this leaves the registration running even after your module is disabled.
...
A standalone slash command like `/mybind` is a plain `mq.bind` — remember to `mq.unbind` it in `Shutdown()` (see above).
