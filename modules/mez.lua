...
local Module   = { _version = '0.1a', _name = "Mez", _author = 'Cannonballdex', }
...
function Module:AEMezCheck()
    ...
    if Config:GetSetting('SafeAEMez') then --not Core.MyClassIs("brd") then
        ...
        else --I think this can all be refactored to something simpler (we need to check from the AutoTarget, which is who we end up casting on), will look later. -- Cannonballdex 1/7/2025
            ...
            if self:IsMezImmune(id) then
                ...
                if data.duration > (spell.MyCastTime() / 100) or spawn.Distance() > Config:GetSetting('MezRadius') or not spawn.LineOfSight() then
                    ...
                    if id == Globals.AutoTargetID then
                        ...
                        -- Cannonballdex note 4/5/2025: This entire thing could likely be refactored. It works much better than before, where we would have ae mez checked once and then we would use 6 single target mezzes instead.
